 ...
 */
struct COAXCAN_EXPORT CanFrame
{
    ...
    {
    	COAXCAN_ASSERT(can_data != COAXCAN_NULLPTR);
        COAXCAN_ASSERT(data_len == dlc);
 ...
 */
struct COAXCAN_EXPORT CanFilterConfig
{
    uint32_t id;
    uint32_t mask;
 ...
 */
struct COAXCAN_EXPORT CanSelectMasks
{
    uint8_t read;
    uint8_t write;
 ...
 */
class COAXCAN_EXPORT ICanIface
{
public:
    virtual ~ICanIface() { }
...
//                            CanIOFlags& out_flags) = 0;
    virtual int16_t receive(coaxcan::CanFrame& out_frame, coaxcan::CanIOFlags& out_flags);
    /**
 ...
 * Generic CAN driver.
 */
class COAXCAN_EXPORT ICanDriver
{
public:
    virtual ~ICanDriver() { }
    ...
    virtual int16_t RXselect(coaxcan::CanSelectMasks& inout_masks) = 0;	//igpark
};
